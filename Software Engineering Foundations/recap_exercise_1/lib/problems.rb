...
def all_vowel_pairs(words)
    ...
    words.each_with_index do |word,ind1|
        words.each_with_index do |word2,ind2|
            if ind1 < ind2 && vowels.all?{|vowel| word.include?(vowel) || word2.include?(vowel)}
                pairs << word + " " + word2
            end
        end
    end
...
def composite?(num)
    (2...num).map {|n| num % n == 0}.include?(true)
end
...
class String
    ...
    def caesar_cipher(num)
        ...
        self.each_char do |char|
            new_ind = (alpha.index(char) + num) % 26
